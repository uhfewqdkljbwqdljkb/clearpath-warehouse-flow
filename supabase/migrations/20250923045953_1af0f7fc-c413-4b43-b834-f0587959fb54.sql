-- Phase 1: Enhanced Role System and Database Foundation (Handle existing policies)
...
-- Update profiles table role column properly
...
-- Recreate companies policies with new role system
CREATE POLICY "Admins can manage all companies" ON companies
  FOR ALL USING (
    public.get_user_role(auth.uid()) IN ('super_admin', 'warehouse_manager')
  );
