...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return <div className={`bg-white h-screen flex flex-col border-r border-gray-200 ${isCollapsed ? 'w-16' : 'w-64'} transition-all duration-300`}>
      {/* Logo Section */}
      ...
      <nav className="flex-1 pt-4">
        <ul className="space-y-1 px-3">
          ...
          return <li key={item.name}>
              ...
              isActive
            }) => `flex items-center px-3 py-2 text-xs font-medium rounded-[8px] transition-colors ${isActive ? 'text-gray-600 hover:text-gray-800' : 'text-gray-600 hover:text-gray-800'}`} style={({
              isActive
            }) => ({
      ...
      <div className="border-t border-gray-200 p-4">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'}`}>
          ...
          {!isCollapsed && <button className="p-1 rounded-md hover:bg-gray-100">
